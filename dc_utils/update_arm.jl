...
function update_arm(rs::Vector{Float64}, qs::Vector{Float64})
    ...
    =#
    link2.r = sa.SVector{3}(rs[1:3])
    link2.q = sa.SVector{4}(qs[1:4])

    link3.r = sa.SVector{3}(rs[4:6])
    link3.q = sa.SVector{4}(qs[5:8])

    link4.r = sa.SVector{3}(rs[7:9])
    link4.q = sa.SVector{4}(qs[9:12])

    link5.r = sa.SVector{3}(rs[10:12])
    link5.q = sa.SVector{4}(qs[13:16])
